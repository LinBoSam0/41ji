...
namespace _41ji
{
    public partial class Form1: Form
    {
        ...
        {
            string currentNumber = txtNumber.Text; // 获取当前文本框中的数字字符串

            // 如果当前数字字符串不为空
            if (!string.IsNullOrEmpty(currentNumber))
            {
                // 计算新的数字字符串长度
                int newLength = currentNumber.Length - 1;
                ...
                {
                    // 否则，使用 Substring 方法获取新的数字字符串
                    txtNumber.Text = currentNumber.Substring(0, newLength);
                }
            }
        }
